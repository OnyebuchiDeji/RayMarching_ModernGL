...
/**
*  Added a chamfer like in an old palace of a medieval era
*   
*/
...
vec2 map(vec3 p)
{
    ...
    vec3 pc = p;
    pc.y -= 3.0;
    float cylinderDist = fCylinder(pc.yxz, 2, 1.5);
    float cylinderID = 3.0;
    vec2 cylinder = vec2(cylinderDist, cylinderID);

    //  walls
    float wallDist = fBox2(p.xy, vec2(1, 7.5));
    float wallID = 4.0;
