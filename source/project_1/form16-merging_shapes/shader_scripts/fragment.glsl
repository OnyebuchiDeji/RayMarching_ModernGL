...
/**
*   Added Box to Scene and Moved the Plane down
*   Added cylinder and merged them using Union operation
*   Swapped coordinate components to rotate the cylinder merged in the box
*/
...
vec2 map(vec3 p)
{
    ...
    //  box
    float boxDist = fBox(p, vec3(1.5, 3, 2));
    float boxID = 3.0;
    vec2 box = vec2(boxDist, boxID);

    //  cylinder
    // // float cylinderDist = fCylinder(p, 2, 3);
    //  Rotating slinder with swizzling operation
    float cylinderDist = fCylinder(p.yxz, 2, 3);
...
void render(inout vec3 col, in vec2 uv)
{
    ...
    if (object.x < MAX_DIST){
        ...
        col += background - max(0.95 * ro.y, 0.0);
    }
}
